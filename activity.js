...
const addEntry = (entry) => bookList[bookList.length] = entry;

const removeEntry = (entry) => {
  const index = bookList.findIndex(book => book.title === entry);
  if (index !== -1) {
    bookList.splice(index, 1)
  }
}

const listBookTitles = () => {for (const key in bookList) {
  console.log(bookList[key].title);
}
}
